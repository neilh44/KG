...
class MultiTimeframePredictor:
    ...
    def fetch_multi_timeframe_data(self):
        ...
        timeframes = {
            '1h': {'hours': 500, 'desc': 'Short-term (1h intervals)'},
            '4h': {'hours': 2000, 'desc': 'Medium-term (4h intervals)'}, 
            '1d': {'hours': 24000, 'desc': 'Long-term (1d intervals)'}
        }
    ...
    def _create_snapshots(self, data: pd.DataFrame, timeframe: str) -> List[Dict]:
        ...
        chunk_size = {'1h': 24, '4h': 6, '1d': 30}.get(timeframe, 24)
        snapshots = []
    ...
    def _get_current_price(self) -> float:
        ...
        if '1h' in self.timeframe_data:
            return float(self.timeframe_data['1h']['close'].iloc[-1])
        return 0.0
